...
class DeepSeekClient:
    ...
    def chat(self, prompt, thinking=False, search=False):
        ...
        thinking_content = ""
        main_content = ""
        current_fragment_type = None  # Track whether we're in THINK or RESPONSE

        def find_fragment_type(obj):
            """Recursively find fragment type in nested structure"""
            if isinstance(obj, dict):
                if "type" in obj and obj["type"] in ["THINK", "RESPONSE"]:
                    return obj["type"], obj.get("content", "")
                for v in obj.values():
                    result = find_fragment_type(v)
                    if result[0]:
                        return result
            elif isinstance(obj, list):
                for item in obj:
                    result = find_fragment_type(item)
                    if result[0]:
                        return result
            return None, None

        def content_generator():
            nonlocal response_message_id, thinking_content, main_content, current_fragment_type
            for line in resp.iter_lines():
                ...
                if decoded.startswith("data:"):
                    ...
                    try:
                        ...
                        # Handle path-based switching observed in logs
                        path = chunk.get("p", "")
                        ...
                        if "v" in chunk:
                            ...
                            if isinstance(v_value, str) and v_value:
                                if current_fragment_type == "THINK":
                                    thinking_content += v_value
                                    yield ("THINK", v_value)
                                elif current_fragment_type == "RESPONSE":
                                    main_content += v_value
                                    yield ("RESPONSE", v_value)
                                else:
                                    # If we haven't seen a path yet but get content,
                                    # it's likely response content if we are not in deep-think mode
                                    # or just a fallback.
                                    main_content += v_value
                                    yield ("RESPONSE", v_value)
                    except Exception:
        ...
        full_content = stream_live(content_generator(), thinking)
